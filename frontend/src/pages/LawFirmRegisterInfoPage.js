...
import {
    ...
    BarChart2, Users, ShieldCheck, Globe, Briefcase, Layers
} from 'lucide-react';
import { Navbar } from '../components/Navbar';
import { useLang } from '../context/LanguageContext';

function FirmDashboardWireframe() {
    return (
        <div className="rounded-2xl overflow-hidden border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#0d1117] shadow-2xl">
            <div className="flex items-center gap-1.5 px-4 py-3 bg-slate-50 dark:bg-[#161b22] border-b border-slate-100 dark:border-white/[0.05]">
                ...
                <span className="flex-1 text-center text-[10px] font-mono text-slate-400 dark:text-slate-600">Firm Dashboard · Lxwyer Up</span>
            </div>
            <div className="flex">
                <div className="w-36 bg-slate-50 dark:bg-[#0a0d12] border-r border-slate-100 dark:border-white/[0.04] p-3 space-y-1">
                    {['Overview', 'Our Lawyers', 'All Cases', 'Revenue', 'Clients', 'Settings'].map((item, i) => (
                ...
                <div className="flex-1 p-4 space-y-3">
                    <div className="grid grid-cols-3 gap-2">
                        {[
                            { l: 'Lawyers', v: '18', d: '+2 this month', c: 'text-slate-900 dark:text-white', blur: false },
                            { l: 'Active Cases', v: '47', d: 'Firm-wide', c: 'text-blue-600 dark:text-blue-400', blur: false },
                            { l: 'Revenue', v: '₹14.2L', d: '+22% MoM', c: 'text-emerald-600 dark:text-emerald-400', blur: true },
                        ].map((s, i) => (
                    ...
                    <div className="rounded-xl border border-slate-100 dark:border-white/[0.05] p-3">
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Lawyer Roster</p>
                        {['Adv. Priya Sharma · 9 cases · Civil', 'Adv. Rahul Mehta · 12 cases · Criminal', 'Adv. Anjali Rao · 6 cases · Corporate'].map((l, i) => (
                            <div key={i} className="flex items-center gap-2 py-1.5 border-b border-slate-50 dark:border-white/[0.04] last:border-0">
                                <div className="w-5 h-5 rounded-full bg-blue-100 dark:bg-blue-500/20 flex items-center justify-center text-[8px] font-bold text-blue-600 dark:text-blue-400 shrink-0">{l[5]}</div>
                                <span className="text-[10px] text-slate-600 dark:text-slate-400 blur-sm select-none">{l}</span>
                            </div>
                        ))}
                    </div>
                </div>
...
export default function LawFirmRegisterInfoPage() {
    const navigate = useNavigate();
    const { t } = useLang();
    useEffect(() => { window.scrollTo(0, 0); }, []);

    const features = [
        { icon: Users,      title: t('lfri_f1_title'), desc: t('lfri_f1_desc') },
        { icon: BarChart2,  title: t('lfri_f2_title'), desc: t('lfri_f2_desc') },
        { icon: Layers,     title: t('lfri_f3_title'), desc: t('lfri_f3_desc') },
        { icon: Globe,      title: t('lfri_f4_title'), desc: t('lfri_f4_desc') },
        { icon: Briefcase,  title: t('lfri_f5_title'), desc: t('lfri_f5_desc') },
        { icon: ShieldCheck,title: t('lfri_f6_title'), desc: t('lfri_f6_desc') },
    ];

    const metrics = [
        { value: '3×',   label: t('lfri_m1') },
        { value: '30%',  label: t('lfri_m2') },
        { value: '99.9%',label: t('lfri_m3') },
    ];
            ...
            <div className="relative pt-32 pb-16 overflow-hidden">
                ...
                <div className="max-w-6xl mx-auto px-6 mb-8">
                    <button onClick={() => navigate('/register')} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors group">
                        <ArrowLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" /> {t('lfri_back')}
                    </button>
                </div>
            ...
            <div className="max-w-6xl mx-auto px-6 pb-24 pt-10">
                ...
                <div className="grid lg:grid-cols-2 gap-14 items-start mb-20">
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                        ...
                        <button onClick={() => navigate('/lawfirm-application')} className="flex items-center gap-2 px-7 py-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm transition-all shadow-lg shadow-blue-600/25 hover:-translate-y-0.5 mb-3">
                            {t('lfri_apply_btn')} <ArrowUpRight className="w-4 h-4" />
                        </button>
                        <p className="text-xs text-slate-400 dark:text-slate-500">{t('lfri_apply_note')}</p>
                    </motion.div>
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
                        <FirmDashboardWireframe />
                        <p className="text-center text-[11px] text-slate-400 dark:text-slate-600 mt-3 tracking-wide">{t('lfri_dashboard_cap')}</p>
                    </motion.div>
                </div>
                ...
                {/* Features */}
                <div className="mb-20">
                    <p className="text-[11px] tracking-[0.22em] uppercase text-blue-600 dark:text-blue-400 font-semibold mb-3">{t('lfri_what_get')}</p>
                    <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-10">{t('lfri_built_for')}</h2>
                        ...
                        {features.map((f, i) => {
                            const Icon = f.icon; return (
                                <motion.div key={i} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.06 }} className="p-6 rounded-xl bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-white/[0.06] hover:border-blue-200 dark:hover:border-blue-500/25 transition-all">
                                    <div className="w-9 h-9 rounded-lg bg-blue-600/10 dark:bg-blue-500/10 flex items-center justify-center mb-4"><Icon className="text-blue-600 dark:text-blue-400" strokeWidth={1.5} size={17} /></div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="relative overflow-hidden rounded-2xl bg-slate-900 dark:bg-blue-950/40 dark:border dark:border-blue-500/20 p-10 md:p-14">
                    ...
                    <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
                        <div>
                            <p className="text-[10px] tracking-[0.2em] uppercase text-blue-400 font-semibold mb-2">{t('lfri_step')}</p>
                            <h3 className="text-3xl font-black text-white mb-2 tracking-tight">{t('lfri_cta_h')}</h3>
                            <p className="text-slate-400 text-sm font-light max-w-md">{t('lfri_cta_sub')}</p>
                        </div>
                        <button onClick={() => navigate('/lawfirm-application')} className="flex items-center gap-2 shrink-0 px-7 py-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-all shadow-lg shadow-blue-600/30 group">
                            <CheckCircle2 className="w-4 h-4" /> {t('lfri_begin')} <ArrowUpRight className="w-4 h-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                        </button>
                    </div>
                </motion.div>
            </div>
