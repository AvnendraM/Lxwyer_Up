                ...
                <div className="space-y-4 relative z-10">
                  {lawyer.achievements.map((ach, i) => (
                    <div key={i} className="bg-white/80 dark:bg-[#1A1A1A] backdrop-blur-md rounded-2xl p-5 border border-yellow-100 dark:border-[#333] flex items-center gap-5">
                      {ach.photo ? (
                        <img src={ach.photo} alt="achievement" className="w-20 h-20 rounded-xl object-cover shrink-0 border border-yellow-200 dark:border-slate-700" />
                      ) : (
                         <div className="w-20 h-20 rounded-xl flex items-center justify-center shrink-0 bg-yellow-100 dark:bg-slate-800 border-yellow-200">
