...
export const InteractiveNeuralVortex = ({ children, className }) => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      float neuro_shape(vec2 uv, float t, float p) {
        ...
        vec2 res = vec2(0.);
        float scale = 8.;
        // Optimization: Reduce loop iterations from 15 to 10 for better performance on high-res screens
        for (int j = 0; j < 10; j++) {
          uv = rotate(uv, 1.);
      ...
      void main() {
        ...
        color = vec3(0.05, 0.25, 0.7);
        color = mix(color, vec3(0.1, 0.5, 0.9), 0.32 + 0.16 * sin(2.0 * u_scroll_progress + 1.2));
        color += vec3(0.0, 0.15, 0.5) * sin(2.0 * u_scroll_progress + 1.5);
        color = color * noise;
    ...
    // Resize handler
    const resizeCanvas = () => {
      // Critical optimization: Limit DPR to prevent severe GPU lag on WebGL fragment shader
